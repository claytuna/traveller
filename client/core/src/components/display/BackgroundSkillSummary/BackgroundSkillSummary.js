	...
	return (
		<Row>
			<Col xs={12}>
				...
				<Row>
					<Col xs={12}>
					{ skills && skills.worldSkills && <Card secondary title="Available Background Skills" body={
						_.size(skills.worldSkills) > 0 ?
							<Row>{_.map(skills.worldSkills, (tr, id)=><Col xs={6} md={4} key={"bg-skil-"+id} title={tr.desc}>{tr.name}</Col> )}</Row> :
